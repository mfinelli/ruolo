...
RSpec.describe Ruolo::Sync do
  ...
  describe '#sync!' do
    before(:all) do
      ...
      described_class.new(
        File.expand_path(File.join(__FILE__, '..', '..', 'fixtures', 'policies',
                                   'sync_spec.yml'))
      ).sync!
    end

    it 'adds missing roles' do
      expect(Ruolo::Models::Role.all.map(&:name)).to eq(%w[EXISTING_ROLE
                                                           NEW_ROLE])
    end
    ...
    it 'adds missing permissions' do
      expect(Ruolo::Models::Permission.all.map(&:name)).to eq(
        %w[EXISTING_PERMISSION NEW_PERMISSION]
      )
    end
    ...
    it 'associates new permissions to a role' do
      %w[EXISTING_ROLE NEW_ROLE].each do |role|
        expect(Ruolo::Models::Role.where(name: role).first.permissions
                                                    .map(&:name)).to eq(
                                                      %w[EXISTING_PERMISSION
                                                         NEW_PERMISSION]
                                                    )
      end
    end
